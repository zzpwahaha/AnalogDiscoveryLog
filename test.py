...
def acquire_and_save_data(fft_analyzer, num_acquisitions, interval, save_raw_data):
    ...
    for _ in range(num_acquisitions):
        ...
        for ch in range(2):
            for acq_idx in range(num_acquisitions):
                # Collect and store raw data and FFT results for each channel
                channel_data = np.array(raw_datas[ch][acq_idx])
                fft_result = np.fft.fft(channel_data, norm='forward')
                magnitude = np.abs(fft_result)
                normalization_factor = np.sqrt(len(channel_data) / fft_analyzer.sample_rate)
                magnitude *= normalization_factor

                data['timestamps'][ch].append(datetime.now().isoformat())
                if save_raw_data:
                    data['raw_data'][ch].append(channel_data.tolist())
                data['fft_results'][ch].append({
                    'frequencies': data['frequency_axis'].tolist(),
...
def main():
    ...
    save_raw_data = True  # Set to True to save raw data, False otherwise
